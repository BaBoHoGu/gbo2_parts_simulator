...
async function runView(view) {
  ...
  try {
    ...
    const ovf = await pg.evaluate(() => {
      const bad = [];
      document.querySelectorAll('.sel-info .panel').forEach(pn => {
        const r = pn.getBoundingClientRect();
        pn.querySelectorAll('*').forEach(e => {
          const q = e.getBoundingClientRect();
          if (q.width && q.right > r.right + 1) bad.push((e.className || e.tagName) + ' +' + Math.round(q.right - r.right));
        });
      });
      return [...new Set(bad)];
    });
    check(view.tag, '[기체정보] 칸 밖으로 넘친 것 없음', ovf.length === 0, ovf.slice(0, 4).join(' | '));
    await step(pg, view, '기체정보');
    if (!(mi.found && mi.h >= mi.need - 2) || ovf.length) await shot(pg, view.tag + '_기체정보');
